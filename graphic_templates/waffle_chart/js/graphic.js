...
var renderWaffleChart = function() {
  ...
  // Show or hide total amounts
  var showTotals = true;
  if (LABELS.showTotals !== "on") showTotals = false;
  ...
  var chartWidth =
    innerWidth - margins.left - margins.right >= parseInt(LABELS.maxWidth)
      ? parseInt(LABELS.maxWidth)
      : innerWidth - margins.left - margins.right;
  var chartHeight = chartWidth;

  var chartSvg = chartWrapper.append("svg").attr({
  ...
  var colorScale = d3.scale.ordinal().range(colorList);

  // Total of all data
  totalAmount = d3.sum(DATA, function(d) {
    return d.amt;
  });
      ...
      selection
        ...
        .attr("fill", function(d, i) {
          switch (d.groupIndex % 4) {
            ...
            case 1:
              return "url(#diagonalHatchLeft)";
            case 2:
              return "rgba(255,255,255,0.0)"; // fully transparent
            case 3:
              return "url(#diagonalHatchRight)";
          }
        })
        .attr("x", function(d, i) {
          col = i % widthSquares;
